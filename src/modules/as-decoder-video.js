...
Module['processHeader'] = function(data, callback) {
	var ret = time(function() {
		...
		return Module['_as_video_decoder_process_header'](buffer, len);
	});
	callback(ret);
};
...
Module['processFrame'] = function(data, callback) {
	var isAsync = Module['_as_video_decoder_async']();
	...
	var ret = time(function() {
		var dest = new Uint8Array(wasmMemory.buffer, buffer, len);
		dest.set(new Uint8Array(data));
		return Module['_as_video_decoder_process_frame'](buffer, len)
	});
	if (!isAsync) {
		callbackWrapper(ret);
	}
...
Module['sync'] = function() {
	...
	if (isAsync) {
		...
		time(function() {
			Module['_as_video_decoder_process_frame'](0, 0)
		});
	}
};
