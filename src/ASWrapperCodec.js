 ...
 * @copyright 2018-2021 yi.cheng
 * @license MIT-style
 */
import ASLoader from './ASLoaderWeb.js';

class ASWrapperCodec {
	constructor(options) {
	...
	init(callback) {
		...
		if (this.options.type === 'video/webm' || this.options.type === 'audio/webm') {
			demuxerClassName = 'ASDemuxerWebMW';
		} else {
			demuxerClassName = 'ASDemuxerOggW';
		}
		ASLoader.loadClass(demuxerClassName, (demuxerClass) => {
	...
	process(callback) {
		if (this.processing) {
			throw new Error('reentrancy fail on ASWrapperCodec.process');
		}
		this.processing = true;
	...
	loadAudioCodec(callback) {
		if (this.demuxer.audioCodec) {
			...
			this.processing = true;
			ASLoader.loadClass(className, (audioCodecClass) => {
				let audioOptions = {};
	...
	loadVideoCodec(callback) {
		if (this.demuxer.videoCodec) {
			...
			let videoClassMap = {
				avc: (threading ? (simd ? 'ASDecoderVideoAvcSIMDMTW'
									 : 'ASDecoderVideoAvcMTW') 
									 :(simd ? 'ASDecoderVideoAvcSIMDW'
									 : 'ASDecoderVideoAvcW')),
				hevc: (threading ? (simd ? 'ASDecoderVideoHevcSIMDMTW'
									   : 'ASDecoderVideoHevcMTW')
							    : (simd ? 'ASDecoderVideoHevcSIMDW'
							           : 'ASVDecoderVideoHevcW')),
				av1: (threading ? (simd ? 'ASDecoderVideoAV1SIMDMTW'
										: 'ASVDecoderVideoAV1MTW')
								: (simd ? 'ASDecoderVideoAV1SIMDW'
										: 'ASDecoderVideoAV1W')),
			};
			let className = videoClassMap[this.demuxer.videoCodec];
			this.processing = true;
			ASLoader.loadClass(className, (videoCodecClass) => {
				let videoOptions = {};
