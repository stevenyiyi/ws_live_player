...
export class NALUAsmHevc {
    ...
    parseSingleNALUPacket(rawData, header) {
        return new HEVC_NALU(header.type,  header.layerid, header.tid, rawData.subarray(0));
    }
    ...
    parseAggregationPacket(rawData, header) {
        ...
        */
        ...
        while (nal_start_idx < data.byteLength) {
            ...
            nal_start_idx++;
            let nalu = this.parseSingleNALUPacket(rawData.subarray(nal_start_idx, nal_start_idx+size), header);
            if (nalu !== null) {
                ret.push(nalu);
            }
            nal_start_idx+=size;
     ...
     */
        ...
        nal_start_idx++;
        if (is_start) {
            this.fragmented_nalu = new HEVC_NALU(payload_type, header.layerid, header.tid, rawData.subarray(nal_start_idx));
        }
    ...
    onNALUFragment(rawData) {
        ...
        if (header.type >= 1 && header.type <= 47) {
            unit = this.parseSingleNALUPacket(rawData.subarray(nal_start_idx), header);
        } else if (HEVC_NALU.FU ===  header.type) {
            unit = this.parseFragmentationUnit(rawData.subarray(nal_start_idx), header);
        } else if (HEVC_NALU.STAP === header.type) {
            return this.parseAggregationPacket(rawData.subarray(nal_start_idx), header);
        } else {
