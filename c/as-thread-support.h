...
void as_video_decoder_init(int width, int height) {
#ifdef __EMSCRIPTEN_PTHREADS__

	init_args_t* args = (init_args_t*)malloc(sizeof(init_args_t));
	args->width = width;
	args->height = height;
	pthread_mutex_init(&decode_mutex, NULL);
	pthread_cond_init(&ping_cond, NULL);
	int ret = pthread_create(&decode_thread, NULL, decode_thread_run,args);
	if (ret) {
		abort();
	}
#else
...
int as_video_decoder_async(void) {
#ifdef __EMSCRIPTEN_PTHREADS__
	return 1;
#else
	return 0;
...
void as_video_decoder_destroy(void) {
	do_destroy();
#ifdef __EMSCRIPTEN_PTHREADS__
	pthread_exit(NULL);
#endif
...
int as_video_decoder_process_header(const char *data, size_t data_len) {
...
int as_video_decoder_process_frame(const char *data, size_t data_len) {
...
static void main_thread_return(void *user_data, float delta) {
	...
	asjs_callback_async_complete(ret, (double)delta);
}

static void *decode_thread_run(void *arg) {
	init_args_t* args = (init_args_t*)arg;
	do_init(args->width, args->height);
	while (1) {
		pthread_mutex_lock(&decode_mutex);
